    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setResult(result.error || t('form.errorMessage'));
            }
        } catch (error) {
        ...
        <div id="contact" className="w-full px-[12%] py-10 scroll-mt-20">
            ...
            <form onSubmit={onSubmit} className="max-w-2xl mx-auto">
                <div className="grid grid-cols-auto gap-6 mt-10 mb-8">
                    <input type="text" placeholder={t('form.namePlaceholder')} className="flex-1 px-3 py-2 focus:ring-1 outline-none border border-gray-300 dark:border-white/30 rounded-md bg-white dark:bg-darkHover/30" required name="name" />

                    <input type="email" placeholder={t('form.emailPlaceholder')} className="flex-1 px-3 py-2 focus:ring-1 outline-none border border-gray-300 dark:border-white/30 rounded-md bg-white dark:bg-darkHover/30" required name="email" />
                </div>
